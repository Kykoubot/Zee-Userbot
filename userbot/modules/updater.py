    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await edit_or_reply(event,
                                ...
                                " untuk dapat deploy perubahan terbaru dari Zee-Userbot.`"
                                )
            repo.__del__()
            return
        ...
        if build.status == "failed":
            ...
            await edit_delete(event,
                              "`Zee-Userbot Berhasil DiUpdate✔️,Restart Harap Tunggu Sebentar`"
                              )

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n" "`Zee-Userbot Berhasil Di Update✅`"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    x = await edit_or_reply(event, "**Zee-Userbot** `Berhasil Di Update!`")
    await asyncio.sleep(1)
    await x.edit("**Zee-Userbot** `Di Restart....`")
    await asyncio.sleep(1)
...
async def upstream(event):
    ...
    if changelog == "" and force_update is False:
        await xx.edit(
            f"\nZee-Userbot Sudah Versi Terbaru✅ || Tunggu Update Terbaru\n"
        )
    ...
    if conf is None and force_update is False:
        ...
        return await event.respond(
            f"**Perintah Untuk Update, Sebagai Berikut.**\n🔰 Ketik: >`{cmd}update now` (Sementara)\n🔰 Ketik: >`{cmd}update deploy` (Permanen)\n\n__Untuk Meng Update Fitur Terbaru Dari Zee-Userbot.__"
        )
    ...
    if force_update:
        ...
        await xx.edit("` Proses Update Zee-Userbot, Loading....1%`")
        await xx.edit("` Proses Update Zee-Userbot, Loading....20%`")
        await xx.edit("` Proses Update Zee-Userbot, Loading....35%`")
        await xx.edit("` Proses Update Zee-Userbot, Loading....77%`")
        await xx.edit("` Proses Update Zee-Userbot, Updating...90%`")
        await xx.edit(
            "` Proses Update Zee-Userbot, Mohon Tunggu Sebentar....100%`"
        )
